		...
		<div class="dgwltd-promo-card__inner">   
			...
			<?php 
				if ($type == 'image' && !empty($image)) {
					require DGWLTD_PLUGIN_BLOCKS . 'build/promo-card/image.php';
				} elseif ($type == 'embed' && !empty($embed)) {
					require DGWLTD_PLUGIN_BLOCKS . 'build/promo-card/embed.php';
				} elseif ($type == 'video' && !empty($video)) {
					require DGWLTD_PLUGIN_BLOCKS . 'build/promo-card/video.php';
				}
			?>
